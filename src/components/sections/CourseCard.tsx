...
export function CourseCard({ course, isOpen, onOpen, onClose }: CourseCardProps) {
  ...
  return (
    <div
      ...
      className={
        isOpen
          ? 'fixed inset-4 z-50 overflow-hidden rounded-3xl border border-gold-400/40 bg-navy-900 shadow-2xl sm:inset-8 md:inset-x-[12%] md:inset-y-10 lg:inset-x-[22%]'
      ...
      {isOpen ? (
        <>
          <button
            ...
            className="absolute right-5 top-5 z-10 flex size-9 cursor-pointer items-center justify-center rounded-full bg-navy-950/70 text-white backdrop-blur-sm transition-colors hover:text-gold-400"
          >
            <CloseIcon className="size-4" />
          </button>

          <div data-lenis-prevent className="h-full overflow-y-auto p-7 sm:p-9">
            <div className="aspect-[16/9] w-full overflow-hidden rounded-2xl">
              <TiltImage src={photo.src} alt={photo.alt} className="size-full" />
            </div>
        ...
        <>
          ...
          <button
            ...
            className="mt-6 flex cursor-pointer items-center gap-2 font-subtitle text-xs font-semibold uppercase tracking-[0.2em] text-gold-400"
          >
            Ver detalhes
